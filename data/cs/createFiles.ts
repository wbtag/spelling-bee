...
const data = readFileSync("./data/cs/czech_words.txt");
const words = data
  .toString()
  .split("\n")
...
if (writeSupplementaryFiles) {
  writeFileSync("./data/cs/answers.txt", validWords.join("\n"));
  writeFileSync("./data/cs/pangrams.txt", pangrams.join("\n"));
}
...
for (let offset = 0; offset < 7; offset++) {
  for (let i = 0; i < numUniqueLetterCombinations; i++) {
    ...
    if (numProcessed % numPuzzlesPerFile === 0) {
      ...
      const fileName = `./data/cs/allAnswers${fileNum === 1 ? '' : fileNum}.json`;
