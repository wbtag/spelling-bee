// This script takes the contents of words.txt
...
const { values } = parseArgs({
  options: {
    locale: { type: "string", short: "v", default: "en"},
  },
});

interface SuppressedCombination {
  letters: string[];
  sampling: number;
}
...
const createAnswers = async () => {
  const currentAnswers = await import(`./${values.locale}/allAnswers.json`);
  ...
  const data = readFileSync(`./${values.locale}/words.txt`);
  const words = data
    .toString()
    .split("\n")
  ...
  const validWords = words.filter((word: string) => {
    ...
    if (uniqueLetters.size < 8) return true;
  });

  const pangrams = validWords.filter((word: string) => new Set(word).size == 7);

  if (writeSupplementaryFiles) {
    writeFileSync(`./${values.locale}/answers.txt`, validWords.join("\n"));
    writeFileSync(`./${values.locale}/pangrams.txt`, pangrams.join("\n"));
  }
  ...
  let numProcessed = 0, numIncluded = 0;

  const suppressedCombinations: SuppressedCombination[] = JSON.parse(readFileSync(`./${values.locale}/suppressedCombinations.json`).toString());
  ...
  for (let offset = 0; offset < 7; offset++) {
    for (let i = 0; i < numUniqueLetterCombinations; i++) {
      ...
      for (const combination of suppressedCombinations) {
        const match = combination.letters.every((letter: string) => availableLetters.includes(letter));
        if (match) {
          const randNum = Math.floor(Math.random() * 100)
          if (randNum >= combination.sampling) {
            include = false;
          }
          break;
        }
      ...
      if (include) {
        ...
        if (numIncluded % numPuzzlesPerFile === 0) {
          let fileNum = numIncluded / numPuzzlesPerFile;

          // validation today and yesterday's puzzle hasn't changed
          const fileName = `./${values.locale}/allAnswers${fileNum === 1 ? '' : fileNum}.json`;
  ...
  console.log(`Processed ${numProcessed} puzzles, included ${numIncluded}.`)
